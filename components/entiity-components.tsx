...
import { AlertTriangleIcon, ListIcon, Loader2Icon, PackageOpenIcon, PlusIcon, SearchIcon, TrashIcon } from 'lucide-react';
...
export const EntityHeader = ({title, description, disabled, isCreating, newButtonHref, newButtonLabel, onNew}: EntityHeaderProps) => {
  return (
...
export const EntityPagination = ({onPageChange, page, totalPages, disabled}: EntityPaginationProps) => {
    return (
        <div className='flex items-center justify-between gap-x-2 w-full'>
            ...
            <div className='flex items-center justify-end space-x-2 py-4'>
                ...
                <Button className='' disabled={page === totalPages || totalPages === 0 || disabled} variant={"outline"} size={"sm"} onClick={() => onPageChange(Math.min(totalPages, page + 1))}>
                    Next
                </Button>
            </div>
        </div>
...
export function EntityList<T> ({
    items, className, renderItem, getKey, emptyView
}: EntityListProps<T>) {
...
export const EntityItem = ({
    href, title, actions, className, image, isRemoving, onRemove,subtitle
}: EntityItemProps) => {
